...
class Flash():
    ...
    def __init__(self, driver, stlink, dbg, bank=0):
        ...
        if self._stlink.target_voltage < 2.0:
            raise src.pystlink.lib.stlinkex.StlinkException('Supply voltage is %.2fV, but minimum for FLASH program or erase is 2.0V' % self._stlink.target_voltage)
        self.unlock()
    ...
    def unlock(self):
        ...
        # programing locked
        if self._stlink.get_debugreg32(Flash.FLASH_CR_REG) & Flash.FLASH_CR_LOCK_BIT:
            raise src.pystlink.lib.stlinkex.StlinkException('Error unlocking FLASH')
    ...
    def wait_for_breakpoint(self, wait_time):
        ...
        while time.time() < end_time and not self._stlink.get_debugreg32(
                src.pystlink.lib.stm32.Stm32.DHCSR_REG) & src.pystlink.lib.stm32.Stm32.DHCSR_STATUS_HALT_BIT:
            time.sleep(wait_time / 20)
...
class Stm32FP(src.pystlink.lib.stm32.Stm32):
    ...
    def flash_write(self, addr, data, erase=False, erase_sizes=None):
        ...
        if addr is None:
            ...
            raise src.pystlink.lib.stlinkex.StlinkException('Start address is not aligned to half-word')
